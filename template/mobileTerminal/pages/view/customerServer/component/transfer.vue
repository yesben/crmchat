...
<script>
import { getStorage, Toast } from 'pages/utils/uniApi.js';
	...
	methods: {
		...
		handleSubmit() {
			http(api.serviceTransfer, {
				kefuToUserId: this.serverList.find(item => item.select).id, // 转接人id
				user_id: getStorage('userData').kefuInfo.user_id
			}).then(res => {
				Toast('转接成功');
