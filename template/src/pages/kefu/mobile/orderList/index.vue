  ...
  <div class="pos-order-list" ref="container">
    <div class="head-box">
      <div class="nav acea-row row-around row-middle">
        <div class="item" :class="where.type === '' ? 'on' : ''" @click="changeStatus('')">
          全部
        </div>
        <div class="item" :class="where.type === 0 ? 'on' : ''" @click="changeStatus(0)">
          未支付
        </div>
        <div class="item" :class="where.type === 2 ? 'on' : ''" @click="changeStatus(2)">
          未收货
        </div>
        <div class="item" :class="where.type === -1 ? 'on' : ''" @click="changeStatus(-1)">
          退款中
        </div>
      </div>
      <div class="input-box">
          ...
          <div class="item" v-for="(item, index) in list" :key="index">
            ...
            <div class="operation acea-row row-between-wrapper">
              ...
              <div class="acea-row row-middle">
                <div class="bnt" @click="modify(item, 0)" v-if="item.paid === 0">
                  一键改价
                </div>
                ...
                <router-link class="bnt" v-if="item._status._type === 1 && item.shipping_type !== 2" :to="'/kefu/orderDelivery/' + item.id + '/' + item.order_id">去发货
                </router-link>
                <div class="bnt cancel" v-if="item._status._type === 1 && item.shipping_type === 2" @click="storeCancellation(item)">
                  去核销
                </div>
              </div>
            </div>
    ...
    <PriceChange v-if="orderInfo" :change="change" :orderInfo="orderInfo" v-on:closechange="changeclose($event)" @closeChange="closeChange($event)" :status="status"></PriceChange>
    <write-off v-if="iShidden" :iShidden="iShidden" :orderInfo="orderInfo" @cancel="cancel" @confirm="confirm"></write-off>
  </div>
</template>
<script>
